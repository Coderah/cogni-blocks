...
import { ReflectionClass, resolveReceiveType } from '@deepkit/type';
...
export class Manager<ExactComponentTypes extends defaultComponentTypes> {
    ...
    createEntity(id = uuid()): typeof this.Entity {
        ...
        const entity = {
            ...
            components: new Proxy(components, {
                set(target, c, value = null) {
                    const componentType = c as keyof ExactComponentTypes;
                    ...
                    if (
                        ...
                        const entityMap =
                            manager.state.entityMap.get(componentType);
                        if (entityMap && entityMap.size > 1) {
                            throw new Error(
                                `[Entity.components.${
                ...
                deleteProperty(target, p) {
                    const componentType = p as keyof ExactComponentTypes;
                    // @ts-ignore
                    entity.previousComponents[componentType] =
                        entity.components[componentType];
                    delete target[componentType];
                    manager.removeEntityMapping(entity, componentType);
                    return true;
                },
            }),
            flagUpdate,
    ...
    for(
        ...
        this.getEntityIds(componentType).forEach((id) =>
            handler(this.getEntity(id) as typeof this.Entity)
        );
    }
